<?php
/**
 * CastaraTabs Controller File.
 *
 * @author   Christopher Ottley <cottley@gmail.com>
 * @license  See attached license file
 */
namespace Concrete\Package\CastaratabsPackage\Block\Castaratabs;
...
class Controller extends BlockController
{
     ...
     * @var string
     */
    protected $btDescription = 'Creates a tab based interface using jQueryUI';
     ...
     * @var string
     */
    protected $btHandle = 'castaratabs';
     ...
     * @var string
     */
    protected $btTable = 'btCastaratabs';
    ...
    public function view()
    {
		...
		//$jscookieversion = '2.1.3';
		
		$jqueryuiversion = $this->jqueryuiversion;
		$jqueryuitheme = $this->jqueryuitheme;
		$jscookieversion = $this->jscookieversion;
		$tabconfig = $this->tabconfig;
		
		$tabconfigrows = explode("\n", $tabconfig);

		$user = new User();
		
		if ($user->isRegistered()) { //User is logged in
          $groups = $user->getUserGroups();
	      $ui = UserInfo::getByID($user->getUserID());
	      $castaratheme = $ui->getAttribute('castaratheme');	
		  if (trim($castaratheme) != '') {
		    $jqueryuitheme = $castaratheme;
		  }			
		}

        $grouptabs = array();
		$groupsubtabs = array();
		foreach ($tabconfigrows as $tabconfigrow) {
          $grouprow = explode(",", $tabconfigrow);
		  $canSeeTab = false;
		  if (trim($grouprow[2]) == '') { // No group membership required to see tab
		    $canSeeTab = true;
		  } else {
		    if ($user->isRegistered()) { //User is logged in
              foreach($groups as $groupID => $groupName) {
				$group = \Concrete\Core\User\Group\Group::getByID($groupID);
                if ($group->getGroupName() == trim($grouprow[2])) {
				  $canSeeTab = true;
				  break;
				}
              }
          ...
          if ($canSeeTab) {
			$grouptabs[$grouprow[0]] = $grouprow[0];
		    $groupsubtabs[$grouprow[0]][] = $grouprow;
		  }
        }		
		$this->set('grouptabs', $grouptabs);
		$this->set('groupsubtabs', $groupsubtabs);
		
        $pg = \Page::getCurrentPage();
        $pgperm = new \Permissions($pg);
		$canEditPage = $pgperm->canEditPageContents() || $pgperm->canEditPageTheme() || $pgperm->canEditPageProperties() || $pgperm->canEditPageTemplate;
        $this->set('canEditPage', $canEditPage);
        $this->set('isEditMode', $pg->isEditMode());	
		
		$html = \Loader::helper('html');
		
		$cjspkg = \Package::getByHandle('castarajs_package');
		$blockURL = $cjspkg->getRelativePath().'/blocks/castarajs/lib';

		$ctabpkg = \Package::getByHandle('castaratabs_package');
		$ctabBlockURL = $ctabpkg->getRelativePath().'/blocks/castaratabs';


        if (!$pg->isEditMode()) {
		  // Add jQuery	
		  $this->addHeaderItem($html->css("{$blockURL}/jquery-ui-themes/{$jqueryuiversion}/themes/{$jqueryuitheme}/jquery-ui.min.css"));
		  $this->addHeaderItem($html->css("{$blockURL}/jquery-ui-themes/{$jqueryuiversion}/themes/{$jqueryuitheme}/theme.css"));
		  
          $this->addHeaderItem($html->javascript("{$blockURL}/jquery-ui/{$jqueryuiversion}/external/jquery/jquery.js"));	
          $this->addHeaderItem($html->javascript("{$blockURL}/jquery-ui/{$jqueryuiversion}/jquery-ui.min.js"));	

          // Add js cookie		  
          $this->addHeaderItem($html->javascript("{$blockURL}/js-cookie/{$jscookieversion}/src/js.cookie.js"));	
		  
          $this->addHeaderItem($html->javascript("{$ctabBlockURL}/auto.js"));		  
    ...
    public function form()
    {
         ...
         */
		 /*
		$this->set('tabgroupheight', $tabgroupheight);	
		$this->set('tabheight', $tabheight);	
		$this->set('jqueryuiversion', $jqueryuiversion);
		$this->set('jqueryuitheme', $jqueryuitheme);
    ...
    /**
     * Run when a block is being imported.
     *
     * @param  Page          $page
